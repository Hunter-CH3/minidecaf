...
/** 语义检查器 */
export class SemanticCheck extends AbstractParseTreeVisitor<Result>
    ...
    visitUnaryExpr(ctx: MiniDecafParser.UnaryExprContext): Result {
        ...
        if (op.text == "&") {
            ...
            this.asReference = false;
            if (!f["lvalue"]) {
                throw new SemanticError(op.start, "lvalue required as unary '&' operand");
            }
            ctx["ty"] = f["ty"].ref();
            if (!ctx["ty"]) {
                // 数组不能取地址，报错
                throw new SemanticError(op.start, `cannot reference type '${f["ty"]}'`);
            }
        } else if (op.text == "*") {
            // 解引用
            ...
            ctx["ty"] = f["ty"].deref();
            if (!ctx["ty"]) {
                throw new SemanticError(op.start, `cannot dereference type '${f["ty"]}'`);
            }
        } else {
            // 其他一元运算
            f.accept(this);
